...
class Manage_torrent_pieces(Thread):
    ...
    def generatePieces(self):
        ...
        for i in range(self.numberOfPieces):
            ...
            if i == (self.numberOfPieces-1):
                pieceLength = self.torrent_information.totalLength - (self.numberOfPieces-1) * self.torrent_information.pieceLength
                pieces.append(Piece.Piece(i, pieceLength, self.torrent_information.pieces[start:end]))
            else:
                pieces.append(Piece.Piece(i, self.torrent_information.pieceLength, self.torrent_information.pieces[start:end]))
        return pieces
    ...
    def getFiles(self):
        ...
        for f in self.torrent_information.fileNames:

            currentSizeFile = f["length"]
            fileOffset = 0

            while  currentSizeFile > 0:
                idPiece = pieceOffset / self.torrent_information.pieceLength
