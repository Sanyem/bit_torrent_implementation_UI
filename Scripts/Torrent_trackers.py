...
class Torrent_trackers(object):
    def __init__(self, torrent, queue_of_new_peers):
    ...
    def getPeersFromTrackers(self):
        for tracker in self.torrent_information.announceList:
            if tracker[0][:4] == "http":
                t1 = FuncThread(self.scrapeHTTP, self.torrent_information,tracker[0])
                self.lstThreads.append(t1)
                t1.start()
            else:
                t2 = FuncThread(self.scrape_udp, self.torrent_information, tracker[0])
    ...
    def parseTrackerResponse(self, peersByte):
        ...
        for i in range(len(raw_bytes) / 6):
            ...
            self.queue_of_new_peers.put([ip, port])
